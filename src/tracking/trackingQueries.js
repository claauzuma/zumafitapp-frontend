  ...
  return useMutation({
    ...
    onMutate: async (variables) => {
      ...
      queryClient.setQueryData(queryKey, (current) => {
        ...
        meals[mealType] = [...(meals[mealType] || []), optimisticLog];
        return recomputeTrackingDay({ ...day, date, meals });
      });
...
function buildOptimisticLog(variables = {}) {
  ...
  const id = `optimistic-${variables.date || "day"}-${variables.mealType || "meal"}-${now}-${Math.random()
    .toString(36)
    .slice(2)}`;
  ...
  return {
    ...
    _id: id,
    date: variables.date,
    mealType: variables.mealType || "snack",
...
function moveAndScaleLog(day, variables = {}) {
  ...
  Object.entries(sourceMeals).forEach(([mealType, items]) => {
    meals[mealType] = [];
    (items || []).forEach((item) => {
  ...
  if (updatedLog) {
    ...
    meals[nextMealType] = [...(meals[nextMealType] || []), updatedLog];
  }
